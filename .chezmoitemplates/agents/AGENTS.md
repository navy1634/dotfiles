...
test-writerはテスト、fixture、mockを先に作り、失敗を確認してから実装担当者へREDを引き継ぎます。実装担当者は仕様と共有契約から最小実装を加え、テストコードを変更しません。evaluator は実装者から独立し、受け入れ条件、プロジェクト全体の DoD、コードレビューの順に検証し、`PASS`、`REVISE`、`REDESIGN` のいずれかを返します。メインセッションは evaluator の結果を材料に最終判定を出します。
